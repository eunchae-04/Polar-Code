...
#define RESULT_FILE RESULT_DIR "/simulation_results.txt"
...
// 표준 Min-Sum SC 디코더
// f-function: sign(l1)*sign(l2) * min(|l1|, |l2|)
// g-function: l2 + (1 - 2*u_hat_left) * l1  (결정 피드백)
...
static void run_gnuplot(void) {
    ...
    fprintf(gp, "set title 'Polar Code (N=1024, K=512) SC Decoder Performance'\n");
...
int main(void) {
    ...
    printf("Running Polar Code (N=%d, K=%d) Min-Sum SC simulation...\n", N, K);
    run_simulation(fp);
    fclose(fp);
